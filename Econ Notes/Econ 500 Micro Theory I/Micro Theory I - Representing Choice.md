...
3. Let $C'(A) = \{ x \in A:  y \in A \implies x \succsim y \}$. Then $C'$ is FNE, by completeness of $\succsim$. $C'$ is coherent: Suppose $x, y \in S \cap T$, with $x \in C(S\cap T)$ and $y \notin C(S \cap T)$. Then by the definition we know that $\neg(y\succsim x)$. Since $x,y$ are both in $S$, $y$ does not satisfy the condition of $C'(S)$. 
4. Suppose $A \subseteq X$, $x,y \in A$, and $x \in C'(A)$. Then $x \succsim y$, and so $u(x) \geq u(y)$. So $x \in \arg \max_{a \in A} u(a)$, so $x \in C(A)$. 
	1. Now suppose $x \in C(A)$. Then $x \in \arg \max_{a \in A} u(a)$. So $y \in A \implies u(x) \geq u(y)$; so $y \in A \implies x \succsim y$, and so $x \in C'(A)$. 
5. If $x \in C(\{x,y\})$, then write $x \succsim_C y$. 
...
```ad-note
title: Finite Representation Theorem: Choice Functions
...
Let $C'$ be the choice function generated by $u$. Then $C'(A) \subseteq C(A)$, by definition. If $x \in C(A)$, then $y \in X \land y \notin C(A)$ means $u(x) > u(y)$, and so $x \in C'(A)$. 
