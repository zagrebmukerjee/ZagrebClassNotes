...
date updated: Friday, June 17th 2022, 1:41 pm
...
1) Write the product in terms of entries of $A$:

$$AX = \begin{bmatrix} 
a_{1,1}- a_{3,1} & a_{1,2} - a_{3,2} & a_{1,3} - a_{1,3} \\
-a_{1,1} + a_{2,1} & -a_{1,2} + a_{2,2} & -a_{1,3} + a_{2,3}  \\
-a_{2,1} + a_{3,1} &-a_{2,2} + a_{3,2}  &-a_{2,3} + a_{3,3}  \\
\end{bmatrix}$$
Elementwise these must all be $0$. Let $a_{1,1}$ be $1$. Then it follows that 
