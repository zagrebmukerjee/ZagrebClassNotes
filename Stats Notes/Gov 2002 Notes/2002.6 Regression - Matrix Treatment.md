...
$$\text{Voting} \sim \beta_0 + \gamma_1 I_\text{Female} + \beta_1 \text{Education} + \beta_2 \text{Age} + \gamma_2(\text{Female} \times \text{Education})$$
...
Then the F-statistic is the relative increase in minimized SSR going from unrestricted to restricted. I.e. how much more of the residual is explained when $H_0$ is allowed to be false. Specifically, 

$$F_0 = \frac{(SSR_R - SSR_{UR})/q}{SSR_{UR}/(n - k - )}$$