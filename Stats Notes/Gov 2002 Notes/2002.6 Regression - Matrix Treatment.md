...
We can then make a vector of $t$ values by comparing each standard error and each $\betahat_i$ to the appropriate null hypothesis. Probably some can be two tailed and some not. Use the $t$ distribution with degrees of freedom $n - k - 1$. Same deal for the confidence intervals. To be clear, I'm not talking about *joint* hypothesis tests, but how to do each on at a tim. 
...
If I want to test a joint hypothesis - that $\betahat_1$ and $\betahat_2$ are jointly distinguished from their nulls -  my test statistic should be compared to a different distribution, the $\chi^2$ distribution. This describes the distribution of the sum of squares of normally distributed RVs: if $Z_1, \ldots, Z_n$ are i.i.d standard normals, then $X = \sum Z_i^2 \sim \chi^2_n$. $E[X] = n, \var(X) = 2n$.

Now, suppose $X_1 \indep X_2$, where $X_1 \sim \chi^2_{df_1}$ and $X_2 \sim \chi^2_{df_2}$. Then we need the F distribution.
...
The null hypothesis here is that $\gamma_1 = \gamma_2 = \gamma_3 = 0$; that is to say, being female has no effect on voting, including through changing the education or age effects. 

So we want to know: Are all the $\gamma$ close to $0$, statistically speaking? What about the combination of $\gamma$? If both, then we fail to reject the null. 

We want to make the <font color=gree>F statistic</font>. To do this, we first fit an unrestricted model: the model above. Then we fit another model, where $H_0$ is true, so $\text{Voting}$ $\sim \beta_0$ $+ \beta_1 \text{Education} + \beta_2 \text{Age}$.
...
With normal errors we can use the F-test at whatever size - without that assumption we can only have convergence.

One commonly used $F$ test is the omnibus test. Test if your coefficients can reject the null hypothesis that they're all zero. 
...
First and last components represent sum of squares of $R\betahat-r$. How far are the restrictions from $0$? Middle component is the variance of $R\betahat-r$, standardizing the sum of squares. Since $\betahat$ is approximately normal, then $R\betahat-r$ is normal, then $W$ is $\chi^2$ distributed. We need 